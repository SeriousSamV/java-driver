...
public class CassandraSchemaParser implements SchemaParser {
  ...
  public CassandraSchemaParser(SchemaRows rows, InternalDriverContext context) {
    this(
        rows,
        context,
        new UserDefinedTypeParser(rows.dataTypeParser(), context),
        new TableParser(rows, context),
        new ViewParser(rows, context),
        new FunctionParser(rows.dataTypeParser(), context),
        new AggregateParser(rows.dataTypeParser(), context));
  }

  protected CassandraSchemaParser(
      SchemaRows rows,
    ...
    this.userDefinedTypeParser = userDefinedTypeParser;
  ...
  protected Map<CqlIdentifier, UserDefinedType> parseTypes(CqlIdentifier keyspaceId) {
    return userDefinedTypeParser.parse(rows.types().get(keyspaceId), keyspaceId);
  }

  protected Map<CqlIdentifier, TableMetadata> parseTables(
  ...
  protected Map<CqlIdentifier, ViewMetadata> parseViews(
      CqlIdentifier keyspaceId, Map<CqlIdentifier, UserDefinedType> types) {
  ...
  protected Map<FunctionSignature, FunctionMetadata> parseFunctions(
      CqlIdentifier keyspaceId, Map<CqlIdentifier, UserDefinedType> types) {
  ...
  protected Map<FunctionSignature, AggregateMetadata> parseAggregates(
      CqlIdentifier keyspaceId, Map<CqlIdentifier, UserDefinedType> types) {
